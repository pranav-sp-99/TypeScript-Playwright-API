...
import { BookingRequestBody } from "../src/test-data/post-data-helper.js";
...
test.describe("Sample test cases for all basic HTTP/HTTPS requests", () => {
  ...
  test("Post API request - Create Booking", async ({ request }) => {
    const { postResponse, bookingData, postRequestBody } =
      await apiHelper.createBooking();
    ...
    expect(bookingData.booking.depositpaid).toEqual(
      postRequestBody.depositpaid
    );
  });

  test("GET API request - Get the Booking", async ({ request }) => {
    const { bookingData, postRequestBody } = await apiHelper.createBooking();
    const { response, responseData } = await apiHelper.getBooking(
  ...
  test("GET API Request with query params - Get the Booking using the values used in the booking", async ({
    request,
  }) => {
    const { bookingData, postRequestBody } = await apiHelper.createBooking();
    const { response, responseData } = await apiHelper.getBookingByName(
      postRequestBody.firstname,
      postRequestBody.lastname
    );

    // Assertions
    expect(response.status()).toBe(200);
    const booking = responseData.find(
      (booking) => booking.bookingid === bookingData.bookingid
    );
  ...
  test("PUT API request - Update the booking details", async ({ request }) => {
    ...
    const { response, updatedData, putRequestBody } =
  ...
  test("DELETE API Request - Deleting a booking", async () => {
    const { bookingData } = await apiHelper.createBooking();
    const token = await apiHelper.generateToken();
    const { response } = await apiHelper.deleteBooking(
      bookingData.bookingid,
      token
    );
